...
async function investigateIP() {
  ...
  try {
    const ipResponse = await fetch('https://aegis-proxy.ka-mixalis99.workers.dev/?url=https://ipwho.is/' + encodeURIComponent(input) + '?fields=status,message,country,countryCode,regionName,city,isp,org,as,proxy,hosting,query,lat,lon');
...
async function checkPublicIP() {
  try {
    ...
    const geoResponse = await fetch('https://aegis-proxy.ka-mixalis99.workers.dev/?url=https://ipwho.is/' + ip + '?fields=country,countryCode,city,isp,proxy,hosting,query');
...
async function loadDashboardIP() {
  try {
    const response = await fetch('https://api64.ipify.org?format=json');
    const ipJson = await response.json();
    const geoResponse = await fetch('https://aegis-proxy.ka-mixalis99.workers.dev/?url=https://ipwho.is/' + ipJson.ip + '?fields=country,countryCode,city,isp,proxy,hosting,query');
    const geo = await geoResponse.json();
